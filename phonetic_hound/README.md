...
### [APK をダウンロード（v0.6.1-dev・10.6 MB）](https://github.com/kijitora-no-hito/android-apps/releases/download/phonetichound-v0.6.1-dev/phonetichound-0.6.1-dev-release.apk)

| 項目 | 内容 |
| --- | --- |
| バージョン | 0.6.1-dev（2026-09-26・開発中） |
| ファイル | `phonetichound-0.6.1-dev-release.apk`（11,073,672 バイト） |
| SHA-256 | `19F43450B0F4784E1EAEABF223DB6C951AAE509F36D67EC09F67E8E7FBAFCB01` |
...
- **新宿の終盤演出「赤い都庁」**: 小基地局 4 局を制圧すると、都庁が赤く染まり、敵が一気に増えます（野良が 2 倍、都庁の方角から群れが押し寄せる）。

### 0.5.1-dev からの変更

- **神城の敵をすべてロボットに**（小型ロボット・指揮ロボット・野良ロボット）。指揮ロボットを倒すと通信端末（スマホと同じ働き）を落とします。
- 神城に**回復スポット（ゲームセンター）**を追加。最初から使える休憩地点で、HP 全回復・セーブ・復活地点になり、周りには敵が近づきません。
  これに合わせて神城のステージを南と東へ広げました（**旧版の神城のセーブは引き継げず、はじめからになります**）。
- 倒した敵が白いまま残ることがある不具合を直しました。
