...
権限は使いません（インターネット通信もしません）。結果の共有は、端末の共有メニューで選んだアプリに画像と文章を渡すだけです。
...
- **スコアアタック**: 「はじめから」でクリアするまでのタイム・撃破数・支配数を記録。ステージごとのランキング（上位 10 件、名前入力付き）、
  リザルト画面の作り直し、スクリーンショット付きの結果共有（#phonetichound）。フリープレイは記録しません。
- 旧版のセーブを「つづきから」でクリアした場合は、タイムが計れていないためランキングの対象外です（はじめからなら記録されます）。
